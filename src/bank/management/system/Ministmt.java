...
public class Ministmt extends JFrame implements ActionListener{
    ...
    Ministmt(String Pinno){
        ...
        try{
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from signup where accpin = '"+Pinno+"'");
            while(rs.next()){
                l3.setText("Card Number:    " + rs.getString("accno").substring(0, 1) + "XXXXXXXX" + rs.getString("cardno").substring(12));
            }
        }catch(Exception e){
            System.out.print(e);
        }
